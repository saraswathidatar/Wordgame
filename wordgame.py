...
#print(get_frequency_dict("yards"))
...
def get_word_score(word, n):
    ...
    res_word = word.lower()
    for x in res_word:
        first_component_score += SCRABBLE_LETTER_VALUES.get(x,0)
    wordlen = len(word)
    second_component = max(1, 7*wordlen - 3*(n-wordlen))
    result = first_component_score * second_component
    return result
#
...
def update_hand(hand, word):
    ...
    for letter in lowercase_word: #if not converted to list,then gives Runtime error:dictionary changed size during iteration
